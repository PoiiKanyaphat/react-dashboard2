...
function App() {
  ...
  useEffect(() => {
    fetch('https://red-violet-ladybug-wrap.cyclic.app/pets_price')
    .then(res => res.json())
    .then(result => {
      console.log(result)
      setPetPrice(result)
    ...
    fetch('https://red-violet-ladybug-wrap.cyclic.app/pets_price_chart')
    .then(res => res.json())
    .then(result => {
      console.log(result)
      setPetChart({
  ...
  return (
    <div>
      <h1>กัญญาพัชร ขาวลูกจันทร์ 6401083</h1>
      <ul>
        {petPrices.map(pet => (
          <li key={pet.id}>
